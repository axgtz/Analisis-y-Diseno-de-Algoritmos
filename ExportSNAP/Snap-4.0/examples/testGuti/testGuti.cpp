//  Graphs SNAP EXPORT
//  Created by Alex Gutz on 10/16/17.
...
int main() {
    ...
    while(!archivo_entrada.eof()){
        ...
        if(iss){//iss es un flujo de strings
            iss >> origen;
            iss >> destino;
            //cout<<"origen "<< origen << "dest "<<destino <<endl;
            dest = stoi(destino);
            org = stoi(origen);
...
//http://changelog.ca/log/2013/03/09/gdf
void exportarGDF(PNGraph Graph){//si Exporta
    ...
    if (archivo_salida.is_open()){
        ...
        /*for (TNGraph::TNodeI NI=Graph->BegNI(); NI<Graph->EndNI(); NI++){
            archivo_salida << NI.GetOutDeg() << "," << NI.GetInDeg() << "\n";
        }*/
...
void exportarGraphMl(PNGraph Graph){//Si exporta
    ofstream archivo_salida;
...
void exportarGEXF(PNGraph Graph){//si Exporta
    ofstream archivo_salida;
